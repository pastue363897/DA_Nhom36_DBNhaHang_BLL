...
public class CustomerDAO extends GeneralCRUD<Customer> {
	...
	private String getLastUsedID() {
		...
		String preparedQL = "select top 1 * from KhachHang where maKH like 'KH%' order by maKH desc";
		try {
	...
	public String addCustomerReturn(Customer customer) {
		AccountDAO accDao = new AccountDAO();
		String id = generateID();
		customer.getTaiKhoan().setMaTK(id);
		accDao.addAccount(customer.getTaiKhoan());
		Session session = sessionFactory.getCurrentSession();
		Transaction tr = session.getTransaction();
		try {
	...
	public List<Customer> timKhachHang(String keyword) {
		...
		try {
			tr.begin();
			String sql = "select * from KhachHang K join TaiKhoan T on K.maKH = T.maTK where ( maKH like 'KH%' AND ( hoTen like '%"
					+ keyword + "%' OR diachi like '%" + keyword + "%' OR cmnd like '%" + keyword + "%' OR sdt like '%"
					+ keyword + "%' OR T.username like '%" + keyword + "%' ) )";
			list = session.createNativeQuery(sql, Customer.class).getResultList();
			tr.commit();
		} catch (Exception e) {
			tr.rollback();
			e.printStackTrace();
